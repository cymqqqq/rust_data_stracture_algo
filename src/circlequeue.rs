...
impl<T> CircleQ<T>{
    ...
    fn push_front_node(&mut self,mut node:Box<Node<T>>){
        ...
        if self.is_empty(){
            ...
            unsafe{
                (*self.head.unwrap().as_ptr()).next=node;
...
impl<T> CircleQ<T>{
    ...
    pub fn split_off(&mut self,at:usize)->CircleQ<T>{
        let len=self.len();
        //handle special case if index is either start or end of list
        if at==0{
    ...
    pub fn remove(&mut self,at:usize)->T{
        ...
        //find node to remove,fron front or from back depending on index location
...
impl<T> IntoIterator for CircleQ<T>{
    type Item=T;
...
impl<'a,T> IntoIterator for &'a CircleQ<T>{
    type Item=&'a T;
...
impl<'a,T> IntoIterator for &'a mut CircleQ<T>{
    type Item=&'a mut T;
...
impl<T:PartialEq> PartialEq for CircleQ<T>{
...
impl<T:Eq> Eq for CircleQ<T>{}
impl<T:PartialOrd> PartialOrd for CircleQ<T>{
    fn partial_cmp(&self,other:&Self)->Option<Ordering>{
        self.iter().partial_cmp(other)
    }
}
impl<T:Ord> Ord for CircleQ<T>{
    #[inline]
    fn cmp(&self,other:&Self)->Ordering{
        self.iter().cmp(other)
    }
}
impl<T:Hash> Hash for CircleQ<T>{
...
unsafe impl<T: Send> Send for CircleQ<T> {}
unsafe impl<T: Sync> Sync for CircleQ<T> {}
...
mod tests {
    ...
    #[test]
    fn push_front() {
        let mut list = CircleQ::<usize>::new();
        for v in 0..100 {
